  ...
  head: () => ({
    meta: [
      { title: "AI Gift Recommender — GiftSphere" },
      {
        name: "description",
        content:
          "Get an AI-curated corporate gifting package for your industry, occasion and budget — in seconds.",
      },
    ...
    try {
      ...
      if ("error" in res) {
        if (res.error === "rate_limited") {
          ...
          toast.error(
            "AI recommender is not configured. Please add XAI_API_KEY, GROK_API_KEY, GROQ_API_KEY, or GEMINI_API_KEY.",
          );
        } else if (res.error === "invalid_api_key") {
          toast.error(
            "AI credentials are invalid. Please check XAI_API_KEY, GROK_API_KEY, GROQ_API_KEY, or GEMINI_API_KEY.",
          );
        } else if (res.error === "unavailable") {
          toast.error("AI service is unavailable. Try again later.");
        } else if (res.error === "parse_failed") {
          toast.error("AI returned an invalid response. Try again.");
          console.error("AI parse failed", res.raw);
        } else if (res.error === "db_error") {
          toast.error("The product catalogue is unavailable. Please try again later.");
        } else if (res.error === "no_products") {
          toast.error("No active gift products are available yet.");
        } else {
          toast.error("Couldn't generate recommendation — try again.");
        }
      } else {
        setResult(res.recommendation);
      }
    } catch (error) {
      console.error("Gift recommendation request failed", error);
      toast.error("Could not reach the AI recommender. Please try again.");
    } finally {
      setLoading(false);
    }
  }
    ...
    <>
      <section className="bg-navy text-ivory">
        <div className="container-page py-16">
          ...
          <h1 className="mt-3 font-display text-4xl md:text-5xl">
            Get a curated gifting package in seconds.
          </h1>
          <p className="mt-3 max-w-2xl text-ivory/75">
            Our AI recommender suggests a tailored package based on your industry, occasion and
            budget. Use it as a starting point — then refine with our team.
          </p>
        </div>
      </section>
      ...
      <section className="py-16">
        <div className="container-page grid gap-10 lg:grid-cols-5">
          <form
            onSubmit={onSubmit}
            className="space-y-5 rounded-2xl border border-border bg-card p-8 shadow-soft lg:col-span-2"
          >
            <div>
            ...
            <div>
              ...
              <Textarea
                ...
                name="notes"
                maxLength={500}
                placeholder="Eco-friendly, photographable, includes hamper..."
                className="mt-1.5"
              />
            </div>
            <Button
              type="submit"
              disabled={loading}
              className="w-full gradient-gold border-0 text-gold-foreground shadow-gold hover:opacity-90"
            >
              {loading ? (
                <>
            ...
            {!result && !loading && (
              <div className="flex h-full min-h-[400px] flex-col items-center justify-center rounded-2xl border border-dashed border-border bg-ivory/30 p-10 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Fill the brief and we'll generate a curated recommendation.
                </p>
              </div>
            )}
            {loading && (
              ...
              <div className="rounded-2xl border border-border bg-card p-8 shadow-elegant">
                ...
                <p className="mt-4 inline-flex items-baseline gap-2 rounded-lg gradient-gold px-4 py-2 text-gold-foreground">
                  ...
                  <span className="font-display text-2xl">
                    ₹{result.estimated_total?.toLocaleString("en-IN")}
                  </span>
                </p>
                ...
                <Button asChild className="mt-6 bg-navy text-navy-foreground hover:bg-navy/90">
                  <a href="/quote">
                    Refine with our team <ArrowRight className="ml-2 h-4 w-4" />
                  </a>
                </Button>
              </div>
            )}
